...
//		try {lines_nb = Files.lines(path).count();}
...
//		
		...
		//HashMap<Set, Integer> A3 = new HashMap<Set, Integer>();
...
//		System.out.println("A2: Map of pairs: ");
//		for(Set<String> seti : A2.keySet()) {
//			int value = A2.get(seti);
//			System.out.println(seti + ": " + value); 
//		}
...
//		
		...
		System.out.println("Associaiton Rules for support = " + support 
				+ " and confidecne = " + confidence+"\n");
		...
		while(s_itr.hasNext()) {
			...
			//fw.write(pair_Set2.toString() + value_of_pair +" -> " + value_of_singleton +"\n");
  			...
  			if( (value_of_pair / value_of_singleton) > confidence) {
  				...
  				System.out.println(pair_Set2 +" -> " + s1  );
		...
		//Pass 3 : Triplets 
		//should we remove pairs with count < support from A2 ???
		//How to create triplets from pairs ??
		
		//Pass on A2 and remove pairs with count < support
		//Pass 3 : Generate triplets and make A3 Map for triplets


		 old_basket = new TreeSet<String>();//Temporal set basket to save items
